...
static struct of_device_id custom_pwm_dt_ids[] = {
    { .compatible = "dev,custom-pwm"},
    { /* end of table */ }
};
// Inform the kernel about the devices this driver supports
MODULE_DEVICE_TABLE(of, custom_pwm_dt_ids);
// Data structure that links the probe and remove functions with our driver
static struct platform_driver pwm_platform = {
    .probe = pwm_probe,
    .remove = pwm_remove,
    .driver = {
        .name = "Custom PWM Driver",
        .owner = THIS_MODULE,
        .of_match_table = custom_pwm_dt_ids
    }
};
// The file operations that can be performed on the custom_pwm character file
static const struct file_operations custom_pwm_fops =
{
    .open = dev_open,
    .owner = THIS_MODULE,
    .read = pwm_read,
...
static int pwm_probe(struct platform_device *pdev)
{
    ...
    dev = devm_kzalloc(&pdev->dev, sizeof(struct custom_pwm_dev), GFP_KERNEL);
    ...
    // Turn the PWM on (access the 0th register in the custom PWM module)
    dev->pwm_value = 0x00;
    ...
    dev->miscdev.name = arr[number_devices-1];//strcat("custom_pwm-",buffer);
    dev->miscdev.fops = &custom_pwm_fops;
...
static ssize_t pwm_read(struct file *file, char *buffer, size_t len, loff_t *offset)
{
    struct custom_pwm_dev *dev = container_of(file->private_data, struct custom_pwm_dev, miscdev);
    int success = 0;

    /* 
    * Get the custom_pwm_dev structure out of the miscdevice structure.
    *
    ...
    * is our custom_pwm_dev structure that has the current led value).
    * 
    ...
    */
    ...
    if(success != 0)
    {
        pr_info("PWM_read: Failed to return current led value to userspace\n");
...
static ssize_t pwm_write(struct file *file, const char *buffer, size_t len, loff_t *offset)
{
    ...
    */
    ...
    if(success != 0)
    {
        pr_info("PWM_write: Failed to read led value from userspace\n");
        return -EFAULT; // Bad address error value. It's likely that "buffer" doesn't point to a good address
    }
    else
    {
...
static int pwm_remove(struct platform_device *pdev)
{
    ...
    struct custom_pwm_dev *dev = (struct custom_pwm_dev*)platform_get_drvdata(pdev);
