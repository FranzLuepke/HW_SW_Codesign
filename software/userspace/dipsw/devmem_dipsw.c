...
int main(int argc, char ** argv)
{
    ...
    printf("This program reads continuously the binary configuration of dip switch and displays it as integer. To finish reading use Ctrl+C.\n");
    printf(" Reading...\n");

    // for(int i = 0; i < 10000; ++i)
    while(1)
    {
        dipsw_value = custom_dipsw_map[0];
        printf("\r Value read: %x", dipsw_value);
        fflush(stdout);
